...
# Создаем датафрейм с удобными именами переменных
houses <- data.frame(
  age = as.numeric(data$Возраст),                                          # Возраст
  metro_distance = as.numeric(data$Дистанция_до_метро),                    # Дистанция до метро
  number_of_convinience_store_close = as.numeric(data$Количество_магазинов_рядом),  # Количество магазинов рядом
  price_per_unit = as.numeric(data$Стоимость_за_ед_площади)                # Стоимость за ед площади
)

# Выводим информацию о данных для проверки
cat("Структура данных:\n")
print(str(houses))
cat("\nПервые несколько строк:\n")
print(head(houses))

# 2. Устанавливаем и подключаем пакет psych
if(!require(psych)) install.packages("psych", repos = "https://cran.r-project.org")
library(psych)

# 3. Подробный дескриптивный анализ
describe(houses)

# 4. Для нормальности Shapiro-Wilk можно отдельно добавить:
apply(houses, 2, shapiro.test)

# Убедимся, что переменные numeric (на всякий случай)
houses$age <- as.numeric(houses$age)
houses$metro_distance <- as.numeric(houses$metro_distance)
houses$number_of_convinience_store_close <- as.numeric(houses$number_of_convinience_store_close)
houses$price_per_unit <- as.numeric(houses$price_per_unit)

# 0.5) Удаление выбросов методом IQR (межквартильный размах)
cat("\n=== Обнаружение и удаление выбросов ===\n")
cat("Исходное количество наблюдений:", nrow(houses), "\n")
...
outliers_age <- detect_outliers_iqr(houses$age, "Возраст")
outliers_metro <- detect_outliers_iqr(houses$metro_distance, "Дистанция до метро")
outliers_stores <- detect_outliers_iqr(houses$number_of_convinience_store_close, "Количество магазинов рядом")
outliers_price <- detect_outliers_iqr(houses$price_per_unit, "Стоимость за ед площади")

# Объединяем все индексы выбросов
all_outliers <- unique(c(outliers_age, outliers_metro, outliers_stores, outliers_price))

cat("\nОбщее количество строк с выбросами (по любой переменной):", length(all_outliers), "\n")
cat("Процент выбросов:", round(100 * length(all_outliers) / nrow(houses), 2), "%\n")
...
# --- Таблицы сопряженности и χ²-тесты для всех пар переменных ---
...
# 5) Корреляционный анализ для выбранных пар переменных
# Судя по диаграммам рассеивания, можно предположить наличие корреляции между:
# 1. "Дистанция до метро" и "Количество магазинов рядом"
# 2. "Дистанция до метро" и "Стоимость за ед. площади"
# 3. "Количество магазинов рядом" и "Стоимость за ед. площади"
# 
# Гипотезы:
# H0: rho = 0 (коэффициент корреляции равен нулю, нет корреляции)
# H1: rho != 0 (коэффициент корреляции не равен нулю, есть корреляция)
